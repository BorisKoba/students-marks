...
public class StudentsServiceImpl implements StudentsService {
	...
	public Mark addMark(long id, Mark mark) {
		...
		if(studentDoc.getMarks().contains(mark)) {
			log.error("student with id{}, mark {} already exsist", id, mark);
			throw new MarkIllegalStateException();
		}
		studentDoc.getMarks().add(mark);
		studentRepo.save(studentDoc);
		log.debug("student id {} with mark {} saved", id, mark);
		return mark;
	}
	...
	public Student updatePhoneNumber(long id, String phoneNumber) {
		StudentDoc studentDoc = studentRepo.findById(id).orElseThrow(()-> new StudentNotFoundException());
		log.debug("student with id{}, old phone number{},new phone number{}", id, studentDoc.getPhone(),phoneNumber);
